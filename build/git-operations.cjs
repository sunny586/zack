...
function gitOperations(GITHUB_FOLDER) {
    try {
        ...
        // 检查是否有未提交的更改
        const statusOutput = execSync('git status --porcelain', { cwd: GITHUB_FOLDER }).toString().trim();
        if (statusOutput) {
            // 如果有未提交的更改，则创建提交信息并提交
            const commitMessage = `update code zack ${dayjs().format('YYYY-MM-DD HH:mm:ss')}`;
            const output2 = execSync(`git commit -m "${commitMessage}"`, { cwd: GITHUB_FOLDER }).toString();
        ...
        const logOutput = execSync('git log HEAD -- origin master', { cwd: GITHUB_FOLDER }).toString().trim();
        if (logOutput) {
            // 如果远程分支有新的提交，则拉取更新
        ...
        // 检查git pull是否导致冲突
        const pullStatusOutput = execSync('git status --porcelain', { cwd: GITHUB_FOLDER }).toString().trim();
